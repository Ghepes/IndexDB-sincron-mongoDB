...
function getTrace() {
  ...
  fetch(`/api/traces/${traceId}`)
    ...
    .catch(err => {
      console.log(err);
      alert('Cannot find a trace with this id! Taking you back.');
      window.history.back();
    });
}

function printTrace(traceData) {
  console.log(traceData);

  traceId = traceData._id;

  const { traceName, createdBy, createdAt, size, logic, comments } = traceData;

  $traceName.textContent = traceName;
  $createdBy.textContent = createdBy;
  $createdAt.textContent = createdAt;
  $size.textContent = size;
  $logicList.innerHTML = logic
...
function handleNewReplySubmit(event) {
  ...
  fetch(`/api/comments/${traceId}/${commentId}`, {
    method: 'PUT',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json'
